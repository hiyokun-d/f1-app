...
export default memo(function TrackMap({
  ...
  const [circuitSvgData, setCircuitSvgData] = useState<{
    paths: string[];
    vbW: number;
    vbH: number;
  } | null>(null);
  ...
  const circuitTransform = useMemo(() => {
    ...
    const pad = 40;
    const s = Math.min(
      (containerW - pad * 2) / vbW,
      (containerH - pad * 2) / vbH,
    );
  ...
  const effectiveOutline =
    (svgOutlineProp?.length ?? 0) >= 2 ? svgOutlineProp! : outline;
  ...
  const pitDrivers = useMemo(() => {
    ...
    for (const p of pits) {
      ...
      if (!cur || p.lap_number > cur.lap_number)
        latestPit.set(p.driver_number, p);
    }
    return new Set(
      [...latestPit.entries()]
  ...
  const { yellowFlagDrivers, hasActiveSC, hasRedFlag } = useMemo(() => {
    if (!raceControl?.length)
      return {
        yellowFlagDrivers: new Set<number>(),
        hasActiveSC: false,
        hasRedFlag: false,
      };
    const latestTime = Math.max(
      ...raceControl.map((rc) => new Date(rc.date).getTime()),
    );
    const cutoff = latestTime - 120_000;
    const recent = raceControl.filter(
      (rc) => new Date(rc.date).getTime() > cutoff,
    );
    const yellowFlagDrivers = new Set(
      recent
        .filter((rc) => rc.flag === "YELLOW" && rc.driver_number != null)
        .map((rc) => rc.driver_number!),
    );
    const hasActiveSC = recent.some(
      (rc) => rc.category === "SafetyCar" || rc.message?.includes("SAFETY CAR"),
    );
  ...
  const zoomToDriver = useCallback(
    (driverX: number, driverY: number) => {
      ...
      const clamped = clampPan(
        (containerW / 2 - driverX) * targetZ,
        (containerH / 2 - driverY) * targetZ,
        targetZ,
      );
  ...
  useEffect(() => {
    if (!circuitSvgUrl) setSvgLoaded(true);
  ...
  const transformTransition = animateTransform
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "#06070a", zIndex: 20 }}
      >
        {circuitSvgUrl && (
          <img
            src={circuitSvgUrl}
            className="absolute inset-0 w-full h-full circuit-svg-art"
        ...
        <div className="relative flex flex-col items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <div
              className="w-1 h-1 rounded-full bg-[#e8002d] animate-pulse"
      ...
      >
            ...
            >
              <span
                style={{
                  ...
                  fontSize: 9,
                  fontWeight: 800,
                  color: hasRedFlag ? "#ef4444" : "#facc15",
                  letterSpacing: "0.3em",
                }}
              >
        ...
        >
          ...
          >
            ...
            {racingLines &&
              racingLines.map(({ driverNumber, points }) => {
            ...
            {livePositions.map((lp) => {
              ...
              const label = isPitting
                ? "PIT"
                : (driver?.name_acronym ?? String(lp.driverNumber));

              const gStyle = isSimulated
                ? {
                    transform: `translate(${lp.x}px, ${lp.y}px)`,
                    transition: SIM_TRANSITION,
              ...
              return (
                <g
                  ...
                  ref={(el) => {
                    if (el) {
                      ...
                      if (
                        !isSimulated &&
                        !driverProxies.current.has(lp.driverNumber)
                      ) {
                        const arc = arcDataRef.current;
                        const ol = outlineRef.current;
                        if (arc && ol.length) {
                  ...
                  style={gStyle}
                  onClick={() => {
                    onSelectDriver(lp.driverNumber);
                    zoomToDriver(lp.x, lp.y);
                  }}
                ...
                >
                  ...
                  {isSelected && (
                    <>
                      <circle
                        r="13"
                        fill="none"
                        stroke={isSimulated ? "#8b5cf6" : fillColor}
                        strokeWidth="1.5"
                  ...
                  <text
                    ...
                    fill={
                      isPitting ? "#f59e0b" : isSelected ? "#fff" : fillColor
                    }
        ...
        {[
          ...
          <div
            ...
            style={{
              border: "2px solid rgba(232,0,45,0.55)",
        ...
        {/* Zoom controls */}
        <div
          className="absolute bottom-4 left-4 flex flex-col gap-1 pointer-events-auto"
          style={{ zIndex: 4 }}
        >
          {["+", "−"].map((lbl, i) => (
            <button
              key={lbl}
              onClick={() =>
                applyZoom(zoom + (i === 0 ? ZOOM_STEP : -ZOOM_STEP))
              }
        ...
        {zoom > 1 && (
          <div
            className="absolute bottom-4 left-14 pointer-events-none flex items-center"
            style={{ zIndex: 4 }}
          >
            <span
              style={{
